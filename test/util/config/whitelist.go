...
type WhitelistConfig struct {
	...
	InternalDBWhiteList           map[string]map[uint64]bool `yaml:"internal_db_white_list"`           // 内网数据库，只允许内部账号访问
}
...
// GetInfinityAttributeLabel 获取无限属性标签白名单
func GetInfinityAttributeLabel(uin string, appBizID uint64) bool {
	if uin == "" {
		return false
	}
	res := false
	if robotIDMap, ok := GetWhitelistConfig().InfinityAttributeLabel[uin]; ok {
		if flag, ok := robotIDMap[DefaultForUin]; ok {
			// 如果有UIN的全局配置，默认使用全局配置
			res = flag
		}
		if flag, ok := robotIDMap[appBizID]; ok {
			// 如果有UIN下的appBizID配置，默认使用appBizID配置
			res = flag
		}
	}
	return res
