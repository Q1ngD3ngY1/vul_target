...
func NewRefer(doc *pb.MatchReferReq_Doc, req *pb.MatchReferReq, robotID, bizID uint64) Refer {
	...
	return Refer{
		...
		Answer:      utils.When(doc.GetDocType() == DocTypeSegment, req.GetAnswer(), doc.GetAnswer()),
		OrgData:     doc.GetOrgData(),
		Mark:        MarkInit,
		SheetInfos:  doc.GetSheetInfo(), // text2Sql的没有RelateID，sheet信息由这里填充
		SessionType: utils.When(req.GetIsRelease(), SessionTypeGreeting, SessionTypeExperience),
		UpdateTime:  now,
		CreateTime:  now,
