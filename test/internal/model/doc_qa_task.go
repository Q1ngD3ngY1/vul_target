...
type DocQATask struct {
	...
	IsDeleted         int       `db:"is_deleted"  gorm:"column:is_deleted"`                     // 是否删除(0未删除 1已删除）
...
// DocQATaskList 文档生成问答任务列表
type DocQATaskList struct {
	ID                uint64    `db:"id"`                   // id
	BusinessID        uint64    `db:"business_id"`          // 对外task_id
	RobotID           uint64    `db:"robot_id"`             // 机器人ID
	DocID             uint64    `db:"doc_id"`               // 文档ID
	CorpID            uint64    `db:"corp_id"`              // 企业ID
	TaskID            uint64    `db:"task_id"`              // 对应定时执行任务表ID
	SourceID          uint64    `db:"source_id"`            // 来源ID，目前只有DiffTask，本身QaTask时为0
	DocName           string    `db:"doc_name"`             // 文档名称
	DocType           string    `db:"doc_type"`             // 文档类型
	QACount           uint64    `db:"qa_count"`             // 生成的问答数
	SegmentCountDone  uint64    `db:"segment_count_done"`   // 已完成问答的切片总数
	SegmentCount      uint64    `db:"segment_count"`        // 文档切片总数
	StopNextSegmentID uint64    `db:"stop_next_segment_id"` // 暂停后下次开始的segment_id
	InputToken        uint64    `db:"input_token"`          // 输入token
	OutputToken       uint64    `db:"output_token"`         // 输出token
	Status            int       `db:"status"`               // 状态(1生成中 2成功 3手动暂停 4资源耗尽暂停 5失败 6手动取消)
	Message           string    `db:"message"`              // 生成失败原因
	IsDeleted         int       `db:"is_deleted"`           // 是否删除(0未删除 1已删除）
	UpdateTime        time.Time `db:"update_time"`          // 更新时间
	CreateTime        time.Time `db:"create_time"`          // 创建时间
}

// ListQaTaskReq 拉取生成问答任务列表请求结构
type ListQaTaskReq struct {
	CorpID   uint64
	RobotID  uint64
	Page     uint32
	PageSize uint32
