...
func New(k int, dataset Observations) (Clusters, error) {
	var c Clusters
	if len(dataset) == 0 || len(dataset[0].Coordinates().Vector) == 0 {
	...
	for i := 0; i < k; i++ {
		var p Coordinates
		for j := 0; j < len(dataset[0].Coordinates().Vector); j++ {
			p.Vector = append(p.Vector, rand.Float64())
		}
...
func (c Cluster) PointsInDimension(n int) Coordinates {
	...
	for _, p := range c.Observations {
		v.Vector = append(v.Vector, p.Coordinates().Vector[n])
	}
	return v
}
...
func (c Clusters) CentersInDimension(n int) Coordinates {
	var v Coordinates
	for _, cl := range c {
		v.Vector = append(v.Vector, cl.Center.Vector[n])
	}
	return v
}
