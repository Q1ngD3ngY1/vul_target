...
// SetMainConfig 设置配置（可用于单元测试时 mock 配置）
func SetMainConfig(c MainConfig) {
	configx.SetWatched(mainConfigKey, c)
}
...
type MainConfig struct {
	...
	RobotDefault                        RobotDefault                     `yaml:"robot_default"`                           // 机器人默认配置
	...
	PublicCorpPrimaryIds                []uint64                         `yaml:"public_corp_primary_ids"`                 // 公开知识库的corpPrimaryId列表，允许越权访问，比如应用模板
	UnfinishedDocParseRefreshConfig     UnfinishedDocParseRefreshConfig  `yaml:"unfinished_doc_parse_refresh_config"`     // 未完成文档解析刷新配置
}
...
type Permissions struct {
	...
	UpdateDBLabelSwitch          bool   `yaml:"update_db_label_switch"`          // 更新数据库标签任务开关
...
func IsVipCorp(uin string) bool {
	for _, vips := range GetMainConfig().VIPGroup {
...
type UnfinishedDocParseRefreshConfig struct {
	Enable                          bool `yaml:"enable"`                               // 是否开启定时任务
	ParseStatusStoreTimeSecond      int  `yaml:"parse_status_store_time_second"`       // 文档解析结果保留时长（单位：秒）
	RefreshBatchSize                int  `yaml:"refresh_batch_size"`                   // 刷新批次大小
	IgnoreFileParseCallbackForDebug bool `yaml:"ignore_file_parse_callback_for_debug"` // 是否忽略文件解析回调
}
