...
func SimpleChat(ctx context.Context, req *LlmRequest) (res *LlmResponse, err error) {
	t0 := time.Now()
	log.DebugContextf(ctx, "SimpleChat call req: %v || chat: %v",
	...
	rsp, err := aiGatewayCli.CreateChatCompletion(ctx, req.chatRequest)
	if err != nil {
		log.ErrorContextf(ctx, "SimpleChat Invoke CreateChatCompletion error: %+v, requestId: %+v", err, req.RequestId)
		return nil, err
	}
	tc := time.Since(t0)
	log.DebugContextf(ctx, "SimpleChat, rsp: %v, ERR: %v, %s",
...
// Chat 流式调用 LLM
func Chat(ctx context.Context, req *LlmRequest, resChan chan *LlmStreamResponse) (err error) {
	defer close(resChan)
	if err = req.ToChatRequest(); err != nil {
		return
	}
	...
	log.DebugContextf(ctx, "llmRepoImplV2|streamChat call req: %v || chat: %v",
		util.Object2String(req), util.Object2StringEscapeHTML(req.chatRequest))
	chatStream, err := aiGatewayCli.CreateChatCompletionStream(ctx, req.chatRequest)
	if err != nil {
		log.ErrorContextf(ctx, "llmRepoImplV2|New chat stream client error: %+v", err)
		return err
	}
	...
	for {
		select {
		case <-ctx.Done():
			log.InfoContextf(ctx, "llmRepoImplV2|streamChat done with context done, requestId: %s", req.RequestId)
			return
		default:
			...
			if recvErr != nil {
				if errors.Is(recvErr, context.Canceled) || strings.Contains(recvErr.Error(), "context canceled") {
					log.WarnContextf(ctx, "llmRepoImplV2|streamChat canceled: %+v, requestId: %s", recvErr, req.RequestId)
				}
				if errors.Is(recvErr, io.EOF) {
					log.WarnContextf(ctx, "llmRepoImplV2|streamChat done with EOF, maybe timeout, requestId: %s", req.RequestId)
					return
				}
			}
			if idx == 0 {
			...
			if idx%(10*logOutput[logSpeed]) == 0 || llmRsp.IsFinished() {
				log.DebugContextf(ctx, "llmRepoImplV2|streamChat[%d] rsp:%s,LLM time cost:%s, total time cost:%v ERR: %v", idx,
			...
			if llmRsp.IsFinished() {
				...
				log.DebugContextf(ctx, "llmRepoImplV2|streamChat done with Finished, requestId: %s", req.RequestId)
				return
			}
		}
	}
