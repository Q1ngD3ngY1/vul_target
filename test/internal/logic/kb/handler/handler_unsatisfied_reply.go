...
func NewUnsatisfiedReplyHandler(kbDao kbdao.Dao) *UnsatisfiedReplyHandler {
...
func (l *UnsatisfiedReplyHandler) CountNeedDeletedData(ctx context.Context, corpID, robotID uint64,
	...
	logx.I(ctx, "UnsatisfiedReplyHandler CountNeedDeletedData, corpID:%d, robotID:%d, tableName:%s",
		corpID, robotID, tableName)
	if tableName != "t_unsatisfied_reply" {
		return 0, fmt.Errorf("tableName must be `t_unsatisfied_reply`")
	}
	return l.kbDao.CountTableNeedDeletedData(ctx, corpID, robotID, "t_unsatisfied_reply")
}
...
func (l *UnsatisfiedReplyHandler) DeleteNeedDeletedData(ctx context.Context, corpID, robotID uint64,
	...
	logx.I(ctx, "UnsatisfiedReplyHandler DeleteNeedDeletedData, corpID:%d, robotID:%d, tableName:%s, "+
		"totalCount:%d", corpID, robotID, tableName, totalCount)
	if tableName != "t_unsatisfied_reply" {
		return fmt.Errorf("tableName must be `t_unsatisfied_reply`")
	}
	deletedCount := int64(0)
	ids, err := l.kbDao.GetCustomFieldIDList(ctx, corpID, robotID, "t_unsatisfied_reply", "id")
	if err != nil {
		return err
	}
	for len(ids) > 0 {
		// t_unsatisfied_reason
		_, err = l.kbDao.DeleteByCustomFieldID(ctx, "t_unsatisfied_reason", 0,
			[]string{"unsatisfied_id"}, []string{"IN"}, []any{ids})
		if err != nil {
			return err
		}
		// t_unsatisfied_reply
		count, err := l.kbDao.DeleteByCustomFieldID(ctx, "t_unsatisfied_reply", int64(len(ids)),
			[]string{"id", "corp_id", "robot_id"}, []string{"IN", "=", "="}, []any{ids, corpID, robotID})
		if err != nil {
			return err
		}
		deletedCount += count
	...
	if deletedCount != totalCount {
		...
		logx.E(ctx, "UnsatisfiedReplyHandler DeleteNeedDeletedData Failed, err:%+v", err)
		return err
	}
	return nil
}
