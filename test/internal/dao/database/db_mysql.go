...
var registerMySQLProxyOnce sync.Once

type DBMySQL struct {
	dao Dao
}

func (d *DBMySQL) GetDBConnection(ctx context.Context, connDbSource entity.DatabaseConn) (*sql.DB, error) {
	...
	if useProxy {
		...
		// 注册自定义MySQL网络连接(使用 sync.Once 保证只注册一次)
		registerMySQLProxyOnce.Do(func() {
			mysql.RegisterDialContext("tcp-proxy", func(ctx context.Context, addr string) (net.Conn, error) {
				return dialer.Dial("tcp", addr)
			})
		})
		dsn = fmt.Sprintf("%s:%s@tcp-proxy(%s:%d)/%s?%s",
			connDbSource.Username, connDbSource.Password, resolvedHost, connDbSource.Port, connDbSource.DbName,
			config.App().DbSource.DsnConfigMysql)
	} else {
	...
	// mysql的dial不支持ctx，必须使用异步的方式测试连接
...
// 原名 GetDBList
func (d *DBMySQL) ShowDatabases(ctx context.Context, connDbSource entity.DatabaseConn) ([]string, error) {
...
func (d *DBMySQL) GetDBTableList(ctx context.Context, connDbSource entity.DatabaseConn, page, pageSize int) (tables []string, total int, err error) {
	ctx, cancel := context.WithTimeout(ctx, config.App().DbSource.ReadConnTimeout)
	defer cancel()
...
func (d *DBMySQL) GetTableInfo(ctx context.Context, connDbSource entity.DatabaseConn, table string) (*entity.TableInfo, error) {
...
func (d *DBMySQL) ListPreviewData(ctx context.Context, connDbSource entity.DatabaseConn,
...
func (d *DBMySQL) CheckDbTableIsExisted(ctx context.Context, dbSource *entity.Database, table string) (bool, error) {
	var varifyTableNameSql string
	var tableSafety string
...
func (d *DBMySQL) GetTopNValueV2(ctx context.Context, dbSource *entity.Database, robotId, dbTableBizID,
	...
	var sqlQuery string
	sqlQuery = fmt.Sprintf("SELECT %s FROM `%s` ORDER BY `%s` LIMIT ? OFFSET ?",
...
func (d *DBMySQL) QuerySqlForDbSource(ctx context.Context, connDbSource entity.DatabaseConn,
	...
	dbConn, err := d.GetDBConnection(ctx, connDbSource)
	if err != nil {
		logx.W(ctx, "QuerySqlForDbSource|get db connection failed: %v", err)
		return nil, nil, errs.ErrOpenDbSourceFail.Error(), err
	}
	defer func() {
		closeErr := dbConn.Close()
		if closeErr != nil {
...
func (d *DBMySQL) QueryDBSchemas(ctx context.Context, connDbSource entity.DatabaseConn) ([]string, error) {
