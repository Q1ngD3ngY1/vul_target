...
func (p SimplePlotter) Plot(cc cluster.Clusters, iteration int) error {
	...
	for i, c := range cc {
		series = append(series, chart.ContinuousSeries{
			...
			XValues: c.PointsInDimension(0).Vector,
			YValues: c.PointsInDimension(1).Vector,
		})
	}
	...
	series = append(series, chart.ContinuousSeries{
		...
		XValues: cc.CentersInDimension(0).Vector,
		YValues: cc.CentersInDimension(1).Vector,
	})
