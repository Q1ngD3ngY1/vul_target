...
type Plotter interface {
	Plot(cc cluster.Clusters, iteration int) error
}
...
func (m Kmeans) Partition(dataset cluster.Observations, k int) (cluster.Clusters, error) {
	if k > len(dataset) {
		return cluster.Clusters{}, fmt.Errorf("the size of the data set must at least equal k")
	}

	cc, err := cluster.New(k, dataset)
	if err != nil {
		return cc, err
	}
