...
type CustUser struct {
	...
	IsDeleted   bool      `db:"is_deleted" gorm:"column:is_deleted"`       // 0正常，1已删除
...
type CustUserFilter struct {
	...
	Offset                int
	Limit                 int
	OrderByModifyTimeDesc bool
	ThirdUserID           string
	Name                  string
	ThirdUserIDLike       string
	NameLike              string
	Query                 string
}
...
type UserRoleFilter struct {
	...
	Types       []uint32 // 类型，0默认，1没传第三方用户id兜底配置，2传了匹配为空兜底设置
	RoleBizIDs  []uint64
	Limit       int
}

// CustUserConfigCache 特殊权限配置 缓存结构
type CustUserConfigCache struct {
	NotSet string `json:"not_set"` // 未传third_user_id时,配置的角色业务id
	NotUse string `json:"not_use"` // 传了third_user_id但角色失效时,配置的角色业务id
}

type CustUserWithRoleInfo struct {
	ID          uint64
	CorpID      uint64
	AppID       uint64
	BusinessID  uint64
	...
	CreateTime  int64
	UpdateTime  int64
	RoleList    []*KnowledgeRole
}

type CustUserConfig struct {
	NotUserIdRoleInfo *KnowledgeRole
	NotUseRoleInfo    *KnowledgeRole
}
