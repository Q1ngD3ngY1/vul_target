...
func (t *CateNode) ToPbCateInfoTree(ctx context.Context, stat map[uint64]uint32) *pb.CateInfo {
	...
	return &pb.CateInfo{
		...
		Total:     stat[t.ID],
		Children:  children,
		CanAdd:    !t.IsUncategorized(ctx) && t.Depth < uint(docEntity.ExcelTplCateLen),
		CanEdit:   !t.IsUncategorized(ctx),
		CanDelete: !t.IsUncategorized(ctx),
	}
}

// ToPbCateInfoTree 转为 PB 分类查询树
func (t *CateNode) ToPbCateQueryTree(ctx context.Context, stat map[uint64]uint32, query string) *pb.CateInfo {
	if t == nil {
		return nil
	}
	// 递归过滤children
	var filteredChildren []*pb.CateInfo
	for _, child := range t.Children {
		if filteredChild := child.ToPbCateQueryTree(ctx, stat, query); filteredChild != nil {
	...
	isMatch := strings.Contains(t.Name, query)
	if isMatch || len(filteredChildren) > 0 {
		logx.D(ctx, "ToPbCateQueryTree: %s, %s, %t", t.Name, query, isMatch)
		return &pb.CateInfo{
			CateBizId: t.BusinessID,
			Name:      t.Name,
			Total:     stat[t.ID],
...
// ToPbQACateQueryTree 转为 PB 分类查询树
func (t *CateNode) ToPbQACateQueryTree(ctx context.Context, stat map[uint64]uint32, query string) *pb.ListQACateRsp_Cate {
	if t == nil {
		return nil
	}
	// 递归过滤children
	var filteredChildren []*pb.ListQACateRsp_Cate
	for _, child := range t.Children {
		if filteredChild := child.ToPbQACateQueryTree(ctx, stat, query); filteredChild != nil {
			filteredChildren = append(filteredChildren, filteredChild)
		}
	}
...
// Path 输出分类节点路径
func (t *CateNode) Path(ctx context.Context, id uint64) ([]string, []uint64) {
	if t == nil {
		return nil, nil
	}
	for _, node := range t.Children {
		if node.ID == id {
			if node.Name == UncategorizedCateName {
				return []string{i18n.Translate(ctx, node.Name)}, []uint64{node.BusinessID}
			}
			return []string{node.Name}, []uint64{node.BusinessID}
		}
		path, bizIds := node.Path(ctx, id)
		if len(path) > 0 {
			if node.Name == UncategorizedCateName {
				return append([]string{i18n.Translate(ctx, node.Name)}, path...), append([]uint64{node.BusinessID}, bizIds...)
			}
			return append([]string{node.Name}, path...), append([]uint64{node.BusinessID}, bizIds...)
		}
	}
	return nil, nil
}
...
func GetCatePath(row []string) (bool, []string) {
	if len(row) < docEntity.ExcelTplCateLen {
		return false, nil
	}
	stop := -1
	for i := docEntity.ExcelTplCateLen - 1; i >= 0; i-- {
